...
# Debian ships these under different binary names. The apt install script also
# symlinks them into ~/.local/bin; these aliases cover boxes where it has not run.
...
# Used by the gmom/gmum aliases below. Defined here because the previous setup
# inherited it from oh-my-zsh, which this config does not use.
git_main_branch() {
