...
if [ -n "$upstream" ]; then
  ...
  [ "${behind:-0}" -gt 0 ] && printf ' #[fg=#94e2d5]⇣%s' "$behind"
  [ "${ahead:-0}" -gt 0 ] && printf ' #[fg=#94e2d5]⇡%s' "$ahead"
fi
...
if git -C "$repo_root" rev-parse --verify refs/stash >/dev/null 2>&1; then
  ...
  [ "$stashes" -gt 0 ] && printf ' #[fg=#94e2d5]*%s' "$stashes"
fi

[ "$conflicted" -gt 0 ] && printf ' #[fg=#f38ba8]~%s' "$conflicted"
[ "$staged" -gt 0 ] && printf ' #[fg=#f9e2af]+%s' "$staged"
[ "$unstaged" -gt 0 ] && printf ' #[fg=#f9e2af]!%s' "$unstaged"
[ "$untracked" -gt 0 ] && printf ' #[fg=#89b4fa]?%s' "$untracked"
