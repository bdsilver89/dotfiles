...
function M.toggle()
  ...
  if M.opts.autoformat then
    utils.info("Enabled format on save", { title = "Formatting" })
  else
    utils.warn("Disabled format on save", { title = "Formatting" })
  end
end
...
function M.notify(formatters)
  ...
  utils.info(lines, { title = "Formatting" })
end
...
function M.get_formatters(bufnr)
  ...
  ---@class Formatters
  local ret = {
    active = {}, ---@type lsp.Client[]
    available = {}, ---@type lsp.Client[]
    null_ls = null_ls,
  }
...
function M.supports_format(client)
  if
    client.config and
    client.config.capabilities and
