...
local function render()
  ...
  local function put(text, hl_group)
    lines[#lines + 1] = text
    if hl_group then
      hl(#lines - 1, 0, #text, hl_group)
    end
  end

  local w = state.winid and vim.api.nvim_win_get_width(state.winid) or 80
  ...
  for _, m in ipairs(marks) do
    vim.api.nvim_buf_set_extmark(state.bufnr, ns, m[1], m[2], { end_col = m[3], hl_group = m[4] })
  end
end
...
local function cursor_plugin()
  ...
  return state.lmap[vim.api.nvim_win_get_cursor(state.winid)[1]]
end
...
local function open()
  ...
  for g, link in pairs(highlights) do
  ...
  vim.bo[state.bufnr].filetype = "packui"
